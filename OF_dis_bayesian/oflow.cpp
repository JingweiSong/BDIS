    ...
    im_bo(im_bo_in), im_bo_dx(im_bo_dx_in), im_bo_dy(im_bo_dy_in)
{
  ...
  for (int sl=op.sc_f; sl>=op.sc_l; --sl)  
  {
    ...
    if (sl < op.sc_f)
    {
      grid_fw[ii]->InitializeFromCoarserOF(flow_fw[ii+1]);
    ...
    {
      grid_fw[ii]->InitializeFromCoarserOF(initflow);
    }
    ...
    if (sl == op.sc_l)
    {
      ...
      if(sl==op.sc_l)
          grid_fw[ii]->AggregateFlowDense(tmp_ptr,tmp_ptr_prob,true,spatial_prob);
      else
          grid_fw[ii]->AggregateFlowDense(tmp_ptr,tmp_ptr_prob,false,spatial_prob);
