...
public:
  ...
  inline const Eigen::Vector2f GetRefPatchPos(int i) const { return pt_ref[i]; }
  inline const Eigen::Vector2f GetQuePatchPos(int i) const { return pat[i]->GetPointPos(); }
  inline const Eigen::Vector2f GetQuePatchDis(int i) const { return pt_ref[i]-pat[i]->GetPointPos(); }



  void AggregateFlowDense(float *flowout,float *probabilityout, const bool bool_last, const float* spatial_prob) const;
