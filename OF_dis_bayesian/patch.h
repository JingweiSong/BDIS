...
public:
  ...
  inline const Eigen::Vector2f GetPointPos() const { return pc->pt_iter; }
  inline const bool IsValid() const { return (!pc->invalid) ; }
        inline const bool SetInValid()  { pc->invalid = false ; }
  inline const float * GetpWeightPtr() const {return (float*) pc->pweight.data(); }

  inline const Eigen::Matrix<float, 1, 1>* GetParam()    const { return &(pc->p_iter); }
  ...
  inline const Eigen::Matrix<float, 1, 1>* GetParamStart() const { return &(pc->p_in); }     
        ...
        inline const float* get_bayesian_prob() const { return bayesian_prob; }
        inline void set_last_level_true()  { last_level = true; }
...
private:
  ...
  bool getPatchStaticNNGrad    (const float* img, const float* img_dx, const float* img_dy,  const Eigen::Vector2f* mid_in, Eigen::Matrix<float, Eigen::Dynamic, 1>* tmp_in,  Eigen::Matrix<float, Eigen::Dynamic, 1>*  tmp_dx_in, Eigen::Matrix<float, Eigen::Dynamic, 1>* tmp_dy_in);

  bool getPatchStaticBil(const float* img, const Eigen::Vector2f* mid_in,  Eigen::Matrix<float, Eigen::Dynamic, 1>* tmp_in_e);
  ...
  patchstate * pc = nullptr;
        ...
        void LossComputeErrorImage_noNaN(Eigen::Matrix<float, Eigen::Dynamic, 1>* patdest, Eigen::Matrix<float, Eigen::Dynamic, 1>* wdest, const Eigen::Matrix<float, Eigen::Dynamic, 1>* patin,  const Eigen::Matrix<float, Eigen::Dynamic, 1>*  tmpin);
        bool last_level;
        int num_valid;
